...
namespace LAB02
{
    ...
    public class Solver
    {
        ...
        {
            // 1. рост из сантиметров в метры
            double heightMeters = heightCm / 100;
            // 2. ИМТ по формуле
            double bmi = weightKg / (heightMeters * heightMeters);
            // 3. Округлить до одного знака после запятой
            double roundedBmi = Math.Round(bmi, 1);
            // 4. Вернуть интерполированную строку
            return $"Рост: {heightCm} см, Вес: {weightKg} кг, ИМТ = {roundedBmi:0.0}";
        }
        ...
        public static string GetTriangleType(double a, double b, double c)
        {
            ...
            return $"Треугольник со сторонами {a}, {b}, {c} - {type}";
        }
        ...
        public static string SecondLargest(int[] array)
        {
            ...
            return $"Второй по величине элемент: {secondMax} (индекс {secondMaxIndex})";
        }
